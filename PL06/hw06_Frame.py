...
def run_func(op_code_node):
    ...
    def cons(node):
        ...
        result=new_l_node
        return create_new_quote_list(result,True)

    def null_q(node):
        #Fill Out
        l_node = run_expr(node.value.next)
        l_node = strip_quote(l_node)

        if l_node.type is not TokenType.LIST:
            return Node(TokenType.FALSE)
        if l_node.value is None:
