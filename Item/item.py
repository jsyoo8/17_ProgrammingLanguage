...
def run_list(root_node):
    ...
    :type root_node: Node
    """
    global lrhs0, Irhs3, check_lamb
    if root_node.value.type is TokenType.LIST:
        lrhs0 = root_node.value.value
        Irhs3 = root_node.value.next
        ...
        if check_lamb == 0 :
            ...
            check_lamb = 1
        # Irhs3 = root_node.value.next
        root_node=run_expr(root_node.value)
        lrhs0 = root_node.value
    elif root_node.value.type is TokenType.LAMBDA:
        lrhs0 = root_node.value
        Irhs3 = root_node.next
...
def run_func(op_code_node):
    ...
    def plus(node):
        ...
        if l_node.value is not None and r_node.value is not None:
    ...
    def lt(node):
        ...
        if l_node.value is not None and r_node.value is not None:
            if (int(l_node.value) - int(r_node.value)) > 0:
                return Node(TokenType.FALSE)
            else:
                return Node(TokenType.TRUE)
    ...
    def not_op(node):
        left = node.value.next
        left = run_expr(left)

        if left is None:
            print("runBinary runExpr null")
            return None
        if left.type is TokenType.TRUE:
            return Node(TokenType.FALSE)
        if left.type is TokenType.FALSE:
            return Node(TokenType.TRUE)
        print("Type Error!")
        return None
    ...
    def define(node):
        ...
        insertTable(l_node, r_node)
        return r_node

    def lamb(node):
        global lrhs0, Irhs3
        rhs0 = copy.deepcopy(lrhs0)
        rhs3 = copy.deepcopy(Irhs3)
        if rhs3 is None:
            lrhs0 = None
            Irhs3 = None
            renode = Node(TokenType.INT, rhs0)
            renode.type = TokenType.LIST
            return renode
        else:
            rhs1 = rhs0.next
            rhs2 = rhs1.next
...
def run_expr(root_node):
    ...
    if root_node.type is TokenType.ID:
        if (LUTable.has_key(root_node.value)):
            temp = LUTable[root_node.value]
            if (type(temp) is str):
...
def insertTable(l_node, r_node):
    temp = {}
    if r_node.type is TokenType.INT:
        r_node = r_node.value
    temp[l_node] = r_node
    LUTable.update(temp)
