...
class Node(object):
    ...
    def __str__(self):
        ...
        if self.type is TokenType.ID:
            ...
            if self.value.type is not TokenType.QUOTE :
                result = '(' + str(self.value) + ')';
            else:
                result = str(self.value);
        else:
            if self.type is TokenType.QUOTE:
                result = self.value;
            else:
...
class BasicPaser(object):
    ...
    def _create_node(self, token):
        ...
        if token is None:
            ...
            apo = Node(TokenType.QUOTE, token.lexeme);
            apo.next = self._parse_expr_list();
            return Node(TokenType.LIST, apo);
    ...
    def _parse_expr_list(self):  # Token이 ‘(‘일 경우, list 형태로 만들어서 반환
...
def Test_BasicPaser():
    test_cute = CuteScanner('( quote ( + 3 2 ) )')
